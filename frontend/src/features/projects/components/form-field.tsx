...
/**
 * The bits every shadcn form control (`Input`/a `Select` trigger) needs
 * wired up to this field's label and message. The old Ark `Field.Root` did
 * this for its children through `useFieldContext()`; Base UI's `Field` is
 * unstyled markup only — no context a plain control reads from — so the
 * caller passes these onto the control by hand instead.
 *
 * Local to projects rather than shared: `@/features/ideas/components/
